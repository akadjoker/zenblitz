...
XFILE_HRESULT xfile_parse_header(parse_buffer * buf, BYTE ** decomp_buffer_ptr)
{
  ...
  if (header[3] != XOFFILE_FORMAT_FLOAT_BITS_32 && header[3] != XOFFILE_FORMAT_FLOAT_BITS_64)
    return XFILE_ERR_BADFILEFLOATSIZE;
  ...
  if (header[2] == XOFFILE_FORMAT_BINARY_MSZIP || header[2] == XOFFILE_FORMAT_TEXT_MSZIP)
  {
    ...
    decomp_buffer = (LPBYTE)malloc(decomp_file_size);
    if (!decomp_buffer)
    {
        
        return XFILE_ERR_BADALLOC;
    }
    *decomp_buffer_ptr = decomp_buffer;
    ...
    while (buf->rem_bytes)
    {
      ...
      err = xfile_mszip_decompress(comp_chunk_size, decomp_chunk_size, (char*)buf->buffer, (char*)decomp_buffer);
      if (err)
      {
    ...
    /* mismatch here was a diagnostic-only ERR() upstream; decompression
       already either succeeded or returned XFILE_ERR_BADALLOC above */
    (void)0;
...
static BOOL is_keyword(parse_buffer* buf, const char* keyword)
{
  ...
  if (!read_bytes(buf, tmp, len))
    return FALSE;
  if (xfile_strnicmp(tmp, keyword, len))
  {
...
static BOOL is_name(parse_buffer* buf)
{
  ...
  tmp[pos < sizeof(tmp) - 1 ? pos : sizeof(tmp) - 1] = 0;
  ...
  while (1)
  {
    ...
    if (check_TOKEN(buf) == TOKEN_NAME)
    {
      ...
      else
      {
        ...
        {
          if (!xfile_stricmp((char*)buf->value, buf->pdxf->xtemplates[cur_member->idx_template].name))
            break;
...
static BOOL check_buffer(parse_buffer * buf, ULONG size)
{
  if ((buf->cur_pos_data + size) > buf->capacity)
  {
    ...
    pdata = (LPBYTE)malloc(new_capacity);
    if (!pdata)
      return FALSE;
    /* buf->pdata is NULL on the very first grow (cur_pos_data is then 0
       too, but a 0-byte memcpy from NULL is still UB) */
    if (buf->pdata)
  ...
  for (i = 0; i < pt->nb_members; i++)
  {
    ...
    for (k = 0; k < nb_elems; k++)
    {
      if (pt->members[i].type == TOKEN_NAME)
      {
        ...
        {
          if (!xfile_stricmp(buf->pdxf->xtemplates[pt->members[i].idx_template].name, buf->pdxf->xtemplates[j].name))
          {
      ...
      else
      {
        ...
        else if (token == TOKEN_LPSTR)
        {
          ...
          if (pt->members[i].type == TOKEN_LPSTR)
          {
            ...
            /* pdata is a packed byte stream, so cur_pos_data is only
               ever 4-byte stepped; storing a pointer straight through
               it is a misaligned write (UB, and a real fault on
               strict-alignment targets). Copy the bytes instead - the
               reader (LoaderX's TextureFilename) memcpy's them back
               out the same way. */
            {
              const char *str_ptr = (const char*)buf->cur_pstrings;
              memcpy(buf->pdata + buf->cur_pos_data, &str_ptr, sizeof(str_ptr));
            }
...
/* public: lets a caller loop "while more top-level objects remain"
   (TOKEN_NONE == 0) the way d3dxof.c's GetNextDataObject checked
   This->buf.rem_bytes before calling parse_object again - peeking the
   token is the check that actually matters, since a file can end in
   trailing whitespace/comments after the last real byte. */
WORD xfile_check_token(parse_buffer *buf) { return check_TOKEN(buf); }
...
BOOL xfile_parse_object(parse_buffer * buf)
{
  ...
  *(DWORD *)(buf->pdata + buf->cur_pos_data) = 0x55555555;
  ...
  {
    if (!xfile_stricmp((char*)buf->value, buf->pdxf->xtemplates[i].name))
    {
