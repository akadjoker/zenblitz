...
;sounds (and related) variables
Global sound_explosion1
Global sound_explosion2
Global sound_explosion_channel
Global sound_icon
Global sound_icon_channel
Global sound_boosters
Global sound_boosters_channel
Global sound_enemy_laser
Global sound_enemy_laser_channel
Global sound_player_laser
Global sound_player_laser_channel
Global sound_hyperjump
Global sound_hyperjump_channel
Global sound_collision
Global sound_collision_channel
Global sound_cloak
Global sound_cloak_channel
Global sound_bullethit1
Global sound_bullethit2
Global sound_bullethit_channel
Global music_menu
Global music_menu_level#
Global music_game
Global music_game_level#
...
;load in the sounds (credit given for .WAV source)
sound_icon = LoadSound("SndRes/icon.wav")					;Mark Rayson
sound_enemy_laser = LoadSound("SndRes/enemy-laser.wav")		;Mark Rayson
sound_hyperjump = LoadSound("SndRes/hyperjump.wav")			;Mark Rayson
SoundPitch sound_hyperjump,32000	;Mark's sample lasted a bit too long in it's original format (22000 Hz), so increasing the Pitch to 32000 Hz makes the sample play faster (and audibly higher too)
sound_collision = LoadSound("SndRes/collision.wav")			;Mark Rayson
sound_explosion1 = LoadSound("SndRes/explosion1.wav") 		;WAV Central (www.wavcentral.com)
sound_explosion2 = LoadSound("SndRes/explosion2.wav") 		;WAV Central (www.wavcentral.com)
sound_boosters = LoadSound("SndRes/boosters.wav")			;WAV Central (www.wavcentral.com)
sound_player_laser = LoadSound("SndRes/player-laser.wav")	;WAV Central (www.wavcentral.com)
sound_cloak = LoadSound("SndRes/cloak.wav")					;WAV Central (www.wavcentral.com)
sound_bullethit1 = LoadSound("SndRes/bullethit1.wav")		;EarthStation1 (www.earthstation1.com)
sound_bullethit2 = LoadSound("SndRes/bullethit2.wav")		;EarthStation1 (www.earthstation1.com)
...
Function menu_loop() ;Start of the Function whose name is "menu_loop()"
	...
	music_menu = PlayMusic("SndRes/Danube.mid")	;start playing the music file "Danube.mid" (found in the SndRes folder) on channel "music_menu"
	...
	Repeat		;a REPEAT.. FOREVER loop will carry out the code in between for as long as the program is running..
		If FLAG_MENUON = 1 Then	;If the FLAG_MENUON flag is set to 1 then "do" the following..
			...
			If ChannelPlaying(music_game) = True Then		;if it is, then..
				music_game_level = music_game_level -.01	;decrease the value of this variable by .01
				ChannelVolume music_game, music_game_level	;set the volume of this channel "music_game" to the value of music_game_level
				If music_game_level <= 0 Then				;if the value of "music_game_level" is 0 or less, then
					StopChannel(music_game)					;stop the sounds playing on channel "music_game"
				End If										;notice I've used an IF statement inside of another IF statement.. You'll be doing this a lot, it's called "Nesting"
			End If
		Else									;if the FLAG_MENUON flag wasn't set to 1 then "do" the following..
			...
			End										;END the program
		End If
		If ChannelPlaying(music_menu) = False Then			;checks to see if the menu music has finished playing on channel "music_menu", if so, restart it!
			music_menu = PlayMusic("SndRes/Danube.mid")
		End If
...
Function game_loop()
	...
	music_game_level = 0.3	;the XM tune I've used is an old Amiga Module from a game called Dynablaster
							;it's a bit loud to go with the game, though, so I want it's volume to be lower than
							;normal.. see in a second..
	sound_hyperjump_channel = PlaySound(sound_hyperjump) 	;The game starts in what seems to be mid-hyperjump, but because
															;this sound is normally called after a keypress, I've got to do it manually
															;just this once! ;)
	Repeat		;as with the menu, we want to cycle though the process of updating/drawing forever, until such time as the game has ended.
		If FLAG_GAMEON = 1 Then		;.. so, if FLAG_GAMEON is 1, then "do" the following.
			...
			If ChannelPlaying(music_menu) = True Then		;this bit slowly turns down the menu music, until the music_menu_level is 0..
				music_menu_level = music_menu_level -.01	
				ChannelVolume music_menu, music_menu_level
				If music_menu_level <= 0 Then				;.. then turns it off completely!
					StopChannel(music_menu)
				End If
			End If
		Else
		...
		End If
		If ChannelPlaying(music_game) = False Then		;once again, this checks if the game music has finished playing
			music_game = PlayMusic("Dynablaster.xm")	;and starts it again..
			ChannelVolume music_game, music_game_level	;remembering that we want to play it quieter, so set it's CHANNELVOLUME to music_game_level (as set a second ago!)
		End If
	Forever
End Function
				...
				If KeyDown(KEY_HYPER)					;if the "HYPER" key is pressed then..
					If PLAYER_JUMPS > 0 Then
						If hypercount = 0 Then				;(if "hypercount" is 0, then we're not already in the middle of a HyperJump, so let's set things off!)
							...
							sound_hyperjump_channel = PlaySound(sound_hyperjump)		;play the Hyperjump soundeffect on the appropriate channel reference
				...
				If hypercount = 0 Then						;as long as we're not in the middle of a hyperjump, do the following..
					...
					If KeyDown(KEY_CLOAK)			;if the CLOAK key is pressed then
						...
						If PLAYER_CLOAK > 2 Then		;if the player has enough (2 points of) PLAYER_CLOAK left, then..
							...
							If ChannelPlaying(sound_cloak_channel) = False Then		;because the sound effect is triggered by a KeyDown event, it will fire repeatedly,
								sound_cloak_channel = PlaySound(sound_cloak)		;which makes an almighty noise as the echo of the sound effect builds on top of 
							End If													;eachother! By using this "If it's playing then do nothing, otherwise Play it again"
																					;peice of code, the sample plays once, then if the key is still held down, then it'll
																					;play it another once.. and so on.. (Take out the IF and the END IF to see what I mean!)
					...
					If KeyDown(KEY_BOOST)							;the BOOST key works in exactly the same way as the CLOAK key
						If PLAYER_BOOSTERS > 2 Then
							...
							End If
							If ChannelPlaying(sound_boosters_channel) = False Then
								sound_boosters_channel = PlaySound(sound_boosters)
							End If
					...
					If KeyDown(KEY_FIRE) Then					;if the "FIRE" key is pressed, then..
						If bulletlimiter = 0 Then					;the "bulletlimiter" value works in exactly the same way as "game_accept_pause" earlier
							...
							sound_player_laser_channel = PlaySound(sound_player_laser)	;play the appropriate sound effect on it's channel
						End If
						FlushKeys
					End If
					...
					If enemy\exp_frame = 0 Then			;as long as this version of "enemies" isn't busy exploding..
						...
						If temp_hyp# < Rnd(1,1.5) And gameovercounter = 0 Then	;if the distance between you and them is less than (a small, but random distance)
							...
							If enemy\bulletlimiter = 0 Then								;exactly like the player shooting, the enemie's bullet needs limiting
								...
								sound_enemy_laser_channel = PlaySound(sound_enemy_laser)
							End If
...
Function game_draw_update()
	...
	ClsColor 0,0,0			;changes the CLearScreen colour to black (0,0,0)
	TileImage game_background,0,0	;This acts as a CLear Screen, even if the Resolution is set bigger than "game_background"'s image size
	...
	End If
		...
		For icon.icons=Each icons
			...
			If checkcol=1 And gameovercounter = 0 Then
				...
				sound_icon_channel = PlaySound(sound_icon)	;plays the "icon" sound on it's appropriate channel
			...
			If enemy\exp_frame = 0 Then
				...
				If checkcol=1 And gameovercounter = 0 Then		;if the two images have collided (this version of "enemies" and the PLAYER)
					...
					If PLAYER_SHIELD < 0 Then			;if the player's shield is less than 0, then
						...
						explosion1or2 = Rnd(0,100)									;there are 2 explosion sound effects (for variation)
																					;this line creates a random number between 0 and 100
																					;(I could just have well have used 0 and 1 and got near
																					;enought the same probability of explosion 1 or 2 variation)
																					
						If explosion1or2 > 49 Then									;if the random number was greater than 49, then..
						
							sound_explosion_channel = PlaySound(sound_explosion1)		;play Explosion sound number 1 on the explosion channel

						Else														;if the number was 49 or less, then..

							sound_explosion_channel = PlaySound(sound_explosion2)		;play Explosion sound number 1 on the explosion channel
						End If
					...
					If ChannelPlaying(sound_collision_channel) = False Then		;once again, because this sample will be triggered multiple times as an enemy image
																				;passes through the player image (i.e.: the enemy really rammed you, as opposed to
																				;just catching you), we must allow for the echoing sample overlap (that we last saw
																				;when the player pressed the BOOST or CLOAK key in game_loop_update() )
																				
																				;so, "As long as the sample isn't already playing.."
						sound_collision_channel = PlaySound(sound_collision)	;play the "Collision" sound on it's appropriate channel reference
					End If
					...
					If enemy\energy <= 0 Then					;check to see if the "enemies"'s energy is 0 or less and
						...
						explosion1or2 = Rnd(0,100)						;work out which of the two explosion sounds we'll use..
						If explosion1or2 > 49 Then
							sound_explosion_channel = PlaySound(sound_explosion1)
						Else
							sound_explosion_channel = PlaySound(sound_explosion2)
						End If
			...
			Select bullets(i,4)		;whose bullet is it? remember when we originally DIMmed this Array, I said that 0 was off, 1 was a PLAYER bullet and 2 was an enemy bullet?
					...
					For enemy.enemies = Each enemies
						If enemy\exp_frame = 0 Then		;.. as long as an enemy isn't already exploding..
							...
							If checkcol = 1 And gameovercounter = 0 Then 	;if the bullet hit the enemy (and the game isn't over) then
								...
								ricochet1or2 = Rnd(0,100)									;similarly to explosions, there are two ricochet sounds
								If ricochet1or2 > 49 Then
									sound_bullethit_channel = PlaySound(sound_bullethit1)
								Else
									sound_bullethit_channel = PlaySound(sound_bullethit2)
								End If
							End If
							If enemy\energy <= 0 Then					;checks to see if we've just killed the enemy!
								enemy\energy = 0
								explosion1or2 = Rnd(0,100)							;if we have, then play 1 of the two explosion sound effects..
								If explosion1or2 > 49 Then
									sound_explosion_channel = PlaySound(sound_explosion1)
								Else
									sound_explosion_channel = PlaySound(sound_explosion2)
				...
				Case 2
					...
					If checkcol = 1 And gameovercounter = 0 Then					;if the bullet hit the player, and the game isn't already over..
						...
						ricochet1or2 = Rnd(0,100)									;play one of the two ricochet sound effects..
						If ricochet1or2 > 49 Then
							sound_bullethit_channel = PlaySound(sound_bullethit1)
						Else
							sound_bullethit_channel = PlaySound(sound_bullethit2)
					...
					If PLAYER_SHIELD < 0 Then		;checks to see if the player's SHIELD is less than 0
						...
						explosion1or2 = Rnd(0,100)									;chooses and plays the sound effect for one of our two explosion noises
						If explosion1or2 > 49 Then
							sound_explosion_channel = PlaySound(sound_explosion1)
						Else
							sound_explosion_channel = PlaySound(sound_explosion2)
	...
	;As we said before, the foremost pictures are drawn last in our Video collage.
	;the following lines draw the "HUD" display stuff like the radar and power bars.
	;All of the images are drawn in Paint Shop and then a Magenta pixel is drawn every-other pixel on the image,
	;so that when drawn on-screen in the program, some of the pixels beneath the image can be seen, wherever
	;there is one of these Magenta (which we made transparent earilier) pixels.
	;this gives a cheap and fast version of Cemi-transparency that you'd probably get in a C++ version of the game.
	DrawImage game_radar,SCREEN_WIDTH-210,3
	DrawImage game_shields,SCREEN_WIDTH-210,223

	;after drawing the "SHIELDS" box, we want to draw "how much" SHIELD the player has left.
	;the "game_levels" image is 192 pixels by 20 pixels in size. Because the PLAYER_SHIELD value
	;runs from 0 to 192 pixels, we can simply draw the first PLAYER_SHIELD value by 20 pixels from the
	;full "game_levels" image.
	;For example, if the player has 100 points of PLAYER_SHIELD left, then we would want to draw the first
	;100*20 pixels from the left of the image. 
	;The DRAWIMAGERECT(rectangle) command will do this for us! It requires the following parameters:
	;Image name, X-coordinate on-screen, Y-coordinate on-screen,
	;top left x and y coordinates on the "game_levels" image and bottom right x and y coordinates
	DrawImageRect game_levels,SCREEN_WIDTH-206,236,0,0,Int(PLAYER_SHIELD),20

	;do the same for CLOAK and BOOSTERS display
	DrawImage game_cloak,SCREEN_WIDTH-210,273
	DrawImageRect game_levels,SCREEN_WIDTH-206,286,0,0,Int(PLAYER_CLOAK),20
	DrawImage game_boosters,SCREEN_WIDTH-210,323
	DrawImageRect game_levels,SCREEN_WIDTH-206,336,0,0,Int(PLAYER_BOOSTERS),20
		...
		;in any Graphics resolution we choose
	
	;the JUMPS box is slightly different, because it is not a solid bar.
	;it is actually 5 different prerendered images	
	DrawImage game_jumps,SCREEN_WIDTH-210,373

	If PLAYER_JUMPS >= 1 Then								;this line says that if the player has 1 or more JUMPS left
		DrawImage game_jumps_level(1),SCREEN_WIDTH-205,389	;then draw the leftmost JUMP indicator (at the calculated position)
	End If
	If PLAYER_JUMPS >= 2 Then								;if the player has 2 or more jumps left then draw
		DrawImage game_jumps_level(2),SCREEN_WIDTH-166,389	;the second JUMP indicator. Obviously, if the player has 2 JUMPs
															;left then he will also have "1 or more jumps left", so the first
															;IF will fire as well as this one..
	End If
	If PLAYER_JUMPS >= 3 Then								;do the same for 3, 4 and 5 (which is the maximum number allowed)
		DrawImage game_jumps_level(3),SCREEN_WIDTH-127,389
	End If
	If PLAYER_JUMPS >= 4 Then
		DrawImage game_jumps_level(4),SCREEN_WIDTH-88,389
	End If
	If PLAYER_JUMPS >= 5 Then
		DrawImage game_jumps_level(5),SCREEN_WIDTH-49,389
	End If
	
	;draw the SCORE and HISCORE boxes
	DrawImage game_score,10,SCREEN_HEIGHT-51
	DrawImage game_hiscore,230,SCREEN_HEIGHT-51

	;remember this from "menu_draw_update()" where we built the HI and LAST SCORES out of the Strings
	;by drawing the first character, then the second etc.. in a horizontally stepped increment?
	;this is exactly the same code, but with slightly different coordinates and a different "font" AnimImage
	;(which is "see through" like the blue boxes)
	For i = 1 To 7
		DrawImage game_scorefont,32+((i-1)*24),SCREEN_HEIGHT-35,Int(Mid(PLAYER_SCORE_STR,i,1))
		DrawImage game_scorefont,252+((i-1)*24),SCREEN_HEIGHT-35,Int(Mid(HI_SCORE_STR,i,1))
	Next
	...
	Next
	For enemy.enemies=Each enemies
		;this draws each of the enemies
