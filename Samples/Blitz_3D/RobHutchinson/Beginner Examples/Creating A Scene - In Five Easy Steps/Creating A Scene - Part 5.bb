;
; SOURCE: CREATING A SCENE - PART 5
; AUTHOR: Rob Hutchinson - 7th October 2001   -  loki.sd@blueyonder.co.uk
; -----------------------------------------------------------------------------------------------
; PART: 5 - "The final effects.."
; -----------------------------------------------------------------------------------------------
; DESCRIPTION:
; As a final touch, we now add some lighting (other than ambient, which is the default),
; and add some 3D sounds effects..
;
; FINAL NOTES:
; Hope this has helped some beginners learn blitz... Peace -- out!
;
...
; Setup the lighting....
AmbientLight 50,50,50   ; Turn off the ambient lighting
Global lightMain = CreateLight(1)
LightColor lightMain,255,0,0;,255,255
;LightRange lightMain,4

; Create a listener for our 3d sound and attach it to our player...
listenerPlayer = CreateListener(sceneCamPiv)

; Load in a couple of sounds to play thru it..
Global soundDance  = Load3DSound("Sounds\WelcomeToTheDance.wav") 
Global soundDoodle = Load3DSound("Sounds\Doodleoo.wav")
...
; Play that corny sound!..
EmitSound soundDance,sceneCamPiv
...
Function UpdateScene()
	...
	If gemstatus = 0
		If EntityDistance(objectGemPiv,sceneCamPiv) <= 3
			...
			; Do checks for LMB		
			If MouseHit(1)
				EmitSound soundDoodle,objectGemPiv
