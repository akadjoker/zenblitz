...
While Not KeyHit(1)
	...
	For k=1 To ticks
		...
		UpdateWorld
		fr=fr+1
		px#=EntityX(player1\entity) : pz#=EntityZ(player1\entity) : py#=EntityY(player1\entity)
		gy#=TerrainY(land,px,0,pz)
		dd#=py-gy
		If fr=1 Then worst#=dd
		If dd<worst Then worst=dd
		If dd<0 Then under=under+1
		If (fr Mod 60)=0 Then Print "fr="+fr+" x="+px+" z="+pz+" playerY="+py+" terr="+gy+" delta="+dd+" colls="+CountCollisions(player1\entity)
		If fr>=480
			Print "PIOR DELTA: "+worst+"   frames dentro do terreno: "+under
			End
		EndIf
